...
from ast import Return, Tuple, Load, Call, Constant, Expr, Index, \
    arguments, arg, FunctionDef, ClassDef, Attribute, Name, Subscript, parse, BinOp, Mult, If
    ...
    body=[
        ...
        FunctionDef(args=arguments(args=[
            ...
            arg(annotation=Subscript(
                ...
                arg='tfds_dir',
                type_comment=None),
            arg(annotation=Subscript(ctx=Load(),
            ...
            body=[Expr(value=Constant(kind=None,
                                      value='\n        Acquire from the official tensorflow_datasets model zoo,'
                                            ...
                                            ':param K: backend engine, e.g., `np` or `tf`.\n\n        '
                                            ':param as_numpy: Convert to numpy ndarrays\n\n        '
                                            ':param data_loader_kwargs: pass this as arguments to'
                                            ' data_loader function\n\n        '
                                            ':return: Train and tests dataset splits.\n        ')),
                  Expr(value=Call(args=[BinOp(left=Constant(kind=None,
                                                            value=5),
                                              op=Mult(),
                                              right=Constant(kind=None,
                                                             value=5))],
                                  func=Name(ctx=Load(),
                                            id='print'),
                                  keywords=[])),
                  If(body=[Expr(value=Call(args=[Constant(kind=None,
                                                          value=True)],
                                           func=Name(ctx=Load(),
                                                     id='print'),
                                           keywords=[])),
                           Return(value=Constant(kind=None,
                                                 value=5))],
                     orelse=[],
                     test=Constant(kind=None,
                                   value=True)),
