...
def column_call_to_param(call):
    ...
    def parse_kwarg(key_word):
        """
        Parse kwarg

        :type key_word: keyword
    ...
    _param = dict(
        filter(
            None,
            chain.from_iterable(
                (map(parse_arg, enumerate(call.args)), map(parse_kwarg, call.keywords))
            ),
        )
    )
    ...
    for shortname, longname in ("PK", "primary_key"), ("FK", "foreign_key"):
